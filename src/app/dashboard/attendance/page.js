      ...
      <div className="desktop-view" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        ...
        <div style={{ position: 'relative', display: 'flex', background: 'rgba(0,0,0,0.2)', padding: '0.35rem', borderRadius: '14px', border: '1px solid rgba(255,255,255,0.05)', width: '320px' }}>
            <div style={{
                ...
                borderRadius: '10px',
                transform: activeTab === 'hourWise' ? 'translateX(0)' : 'translateX(100%)',
                willChange: 'transform',
                transition: 'transform 0.3s cubic-bezier(0.16, 1, 0.3, 1), opacity 0.5s cubic-bezier(0.22, 1, 0.36, 1), background-color 0.5s cubic-bezier(0.22, 1, 0.36, 1), border-color 0.5s cubic-bezier(0.22, 1, 0.36, 1)'
            }} />
            <button 
                onClick={() => { setSlideDirection('left'); setActiveTab('hourWise'); }}
                style={{ textAlign: 'center', width: '100%', position: 'relative', zIndex: 1, padding: '0.5rem 1rem', borderRadius: '10px', border: 'none', background: 'transparent', color: activeTab === 'hourWise' ? 'white' : 'rgba(255,255,255,0.6)', fontWeight: '600', cursor: 'pointer', transition: 'color 0.3s', fontSize: '0.9rem' }}
            >
                Hour Wise
            </button>
            <button 
                onClick={() => { setSlideDirection('right'); setActiveTab('subjectWise'); }}
                style={{ textAlign: 'center', width: '100%', position: 'relative', zIndex: 1, padding: '0.5rem 1rem', borderRadius: '10px', border: 'none', background: 'transparent', color: activeTab === 'subjectWise' ? 'white' : 'rgba(255,255,255,0.6)', fontWeight: '600', cursor: 'pointer', transition: 'color 0.3s', fontSize: '0.9rem' }}
            >
                Subject Wise
            </button>
        </div>
